...
class App {
  ...
  constructor(routes: Routes[]) {
    ...
    this.port = BATCHES_SERVICE_PORT || 3003;
  ...
  public listen() {
    ...
    this.app.listen(this.port, () => {
      console.log(`Listening Batches at http://localhost:${this.port}/api`);
      logger.info(`=================================`);
      logger.info(`======= ENV: ${this.env} =======`);
      logger.info(`🚀 Batches App listening on the port ${this.port}`);
      logger.info(`=================================`);
    });
  }
  ...
  private initializeRoutes(routes: Routes[]) {
    routes.forEach((route) => {
      this.app.use('/api/batches', route.router);
    });

    this.app.use('*', function (req, res) {
      console.log('yyyyyyy', req.url);
