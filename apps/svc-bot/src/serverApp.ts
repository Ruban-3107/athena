...
import { errorMiddleware } from '@athena/shared/middleware';
...
class App {
  ...
  constructor(routes: Routes[]) {
    ...
    this.port = BOT_SERVICE_PORT || 3004;
  ...
  private initializeRoutes(routes: Routes[]) {
    routes.forEach((route) => {
      this.app.use('/api/bot', route.router);
    });

    this.app.use('*', function (req, res) {
      console.log('fffffffffffffff', req.url);
